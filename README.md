...
oc create -f https://raw.githubusercontent.com/albertoclarit/kube-cleanup-operator/master/deploy/rbac.yaml

# create deployment
oc create -f https://raw.githubusercontent.com/albertoclarit/kube-cleanup-operator/master/deploy/deployment.yaml


oc logs -f $(kubectl get pods --namespace default -l "run=cleanup-operator" -o jsonpath="{.items[0].metadata.name}")

# Use simple job to test it
oc create -f https://raw.githubusercontent.com/kubernetes/kubernetes.github.io/master/docs/concepts/workloads/controllers/job.yaml
```
